...
Tuy nhiên, ngay bước đầu gọi lệnh `fetch()` để kéo data từ Google Sheet về, trình duyệt đã báo lỗi `CORS Policy Error`. Mình loay hoay mất một lúc để xử lý header, vì Google Sheet không hỗ trợ mở CORS dễ dàng như API thông thường.

Cuối cùng, mình nhớ ra một giải pháp: xử lý qua cơ chế **JSONP**. Ý tưởng là chèn một thẻ `<script>` vào DOM để trình duyệt tự tải dữ liệu thay vì dùng hàm fetch, như sau:
...
Và đây là đoạn cần lưu ý nhất: Tính góc xoay (`rotation`). Để vòng quay chạy trơn tru, mình kết hợp một chút công thức toán học vào hàm Svelte state.
...
Khúc mất thời gian nhất là tìm được đường cong `cubic-bezier(0.2, 0.8, 0.2, 1)`. Chỉnh đi chỉnh lại nhiều lần thì mới có cảm giác quay nhanh dần đều rồi dừng lại từ từ giống hệt vòng quay thực tế. Quả thực gõ code thì ít mà ngồi tinh chỉnh giao diện khá tốn thời gian.
...
Thấy làm mấy cái phần mềm cá nhân nhỏ nhỏ dọn gọn gàng nhu cầu thế này công nhận giúp ích được rất nhiều. Bạn nào cũng muốn giải quyết nhanh câu hỏi ăn gì mỗi ngày giống mình, có hứng thì ghé qua repo Github lấy mã nguồn về trổ tài thử nhé. Hẹn mọi người ở một dự án tự build khác!
